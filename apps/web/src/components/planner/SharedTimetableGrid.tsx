...
export function SharedTimetableGrid({ placed, onCourseClick }: { placed: PlacedCourse[]; onCourseClick?: (offeringId: string) => void }) {
  ...
  return (
    ...
    >
      ...
      {tokens.map((tok) => (
        <ReadonlyRow key={tok} token={tok} start={startOf.get(tok)} days={weekDays} placed={placed} byId={byId} onCourseClick={onCourseClick} />
      ))}
    </div>
  );
}
...
function ReadonlyRow({
  ...
  onCourseClick?: (offeringId: string) => void;
}) {
  return (
    <>
      ...
      {days.map((d) => {
        ...
        const clickable = ids.length >= 1 && !!onCourseClick;
        return (
          <div
            key={`${d}-${token}`}
            role={clickable ? "button" : undefined}
            tabIndex={clickable ? 0 : undefined}
            aria-label={clickable ? first?.name.zh : undefined}
            onClick={clickable ? () => onCourseClick?.(ids[0]) : undefined}
            onKeyDown={clickable ? (ev) => { if (ev.key === "Enter" || ev.key === " ") { ev.preventDefault(); onCourseClick?.(ids[0]); } } : undefined}
            className={cn(
              ...
              clickable && "cursor-pointer outline-none transition hover:brightness-105 focus-visible:ring-2 focus-visible:ring-white/80",
            )}
          >
            {first && (
