...
**不得 commit 任何個資**：學號、特定學生的班級/cunum/可選課程、帳密、session、`.env`。
這些多半來自開發者用真實學校帳號測試後的衍生產物——寫測試/貼範例前先去識別化。
`docs/` 已去識別化；新增內容也須保持。

**這條不涵蓋的東西**（別誤推）：
- **執行期的匿名行為統計**（GA4 等）不受本條限制。使用者查課、加入課表、瀏覽課程
  這類操作是產品的正常分析對象——未來的「熱門課程」等功能本來就需要課程層級的
  彙總資料，不需要無謂的去識別化。GA 的收送規範另見 `apps/web/src/lib/analytics/`。
- **課程目錄資料本身**（課名、教師、時段、課綱）是校方公開資訊，不是個資。
