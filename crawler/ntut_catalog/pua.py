...
背景（2026-07-19 診斷）：學校資料含瀏覽器無字型可畫的 PUA 字元，三類：
  ...
  2. 學校造字（U+E000–E2FF，73 碼位）：教師名/notes/課名。字形一律取自學校 GServer
...
PUA_MAP: dict[int, str] = {
    ...
    # 2026-09 round 2：這三個的 Wingdings 字形名（envelopeback／budleafse／oneoclock）
    # 與用法對不上——原文一律當條列項目符號用，字形本身不帶語意。曾考慮照 Symbol 解讀
    # （Symbol 0xB7 = • BULLET，pua.py 對 0xF0B1 有取 Symbol 的先例），使用者判讀後決定
    # 三個一律視為空白；請勿「修正」成項目符號。
    0xF02A: " ",   # Wingdings 0x2A = envelopeback；用於「參考書：」後的條列 → 空格
    0xF099: " ",   # Wingdings 0x99 = budleafse；用於評分項目條列 → 空格
    0xF0B7: " ",   # Wingdings 0xB7 = oneoclock；用於課程目標條列 → 空格
    # ── (2) 學校造字（字形認定，非猜測） ──
    ...
    0xE019: "鑛",  # GServer 字形認定；課綱「…(中譯本) 機率論，朱蘊▨ 譯 (華泰)…」（機率與統計）
    ...
    0xE069: "參",  # GServer 字形為「叁」（三的大寫），依使用者判讀正規化為通用字「參」；課綱「…公司治理10.05 ▨、倫理議題分析與企…」為壹貳參編號（企業倫理）
    0xE06E: "玎",  # GServer 字形認定；課綱「… 廖凰玎，20…」（課號364509）
    0xE077: "姵",  # GServer 字形認定；課綱「…曄、丁姵如,法…」（課號325274）
    0xE079: "數",  # GServer 字形為「数」（簡化字形），依使用者判讀正規化為通用字「數」；課綱「…(陳銘崑) ▨字化供是什么？…」＝數字化（作業管理）
    ...
    0xE0A5: "焄",  # GServer 字形認定；課綱「…5.童國倫、張楷▨、林義峯，淡定學Endnote…」（資訊蒐集與論文寫作）
    ...
    0xE0C8: "運",  # GServer 字形為「运」（簡化字形），依使用者判讀正規化為通用字「運」；課綱「…企供管理与▨…」（作業管理）
    ...
    0xE19D: "菓",  # GServer 字形認定；課綱「…陳玉箴〈漢餅、台灣▨子、麵包〉…」＝台灣菓子（飲食文化）
