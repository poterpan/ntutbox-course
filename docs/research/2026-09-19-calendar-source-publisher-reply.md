...
**manifest 收錄暫緩**：`ManifestTerm.catalog` 是必填，要收得動 schema 與你們的 decoder。
`calendar.json` 只有 1.8 KB、`max-age=86400`、有 ETag，直接 conditional GET 即可。
如果你們確定需要經 manifest，跟我說，那要當破壞性變更排。
