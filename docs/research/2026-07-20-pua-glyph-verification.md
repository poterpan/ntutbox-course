...
- **2026-09 續作（§6）**：清空 `pua_scan` 累積的 **51 個待辦碼位**——23 個學校造字（有 GServer 輪廓）
  與 22 個 Wingdings 符號入表，6 個字型子集殘留不入表。`PUA_MAP` 現共 **107 碼位**
  （學校造字 73／Word 符號 34），全 11 學期實跑掃描 0 命中，恢復「有命中＝真的有待辦」。
...
| 組別 | 數量 | GServer 字形 | 處置 |
|---|---|---|---|
| 學校造字 `E000–E2FF` | 23（+6 見 §6.4） | **有輪廓** | 使用者逐碼目視判讀 → 入 `PUA_MAP` |
| Word/Wingdings `F020–F0FF` | 22（+3 見 §6.4） | 無 | 依 Wingdings 字形名對應 → 入 `PUA_MAP` |
...
### 6.4 round 2：又冒出來的 9 碼

第一批 51 碼的清單是在**歷史 backfill 完成前**產生的；110-1／110-2 補齊後再掃，又出現 9 個。
教訓：**待辦清單要在資料補齊後重新產生**，否則會以為做完了。

6 個 E 區（`E019` `E069` `E079` `E0A5` `E0C8` `E19D`）全部有 GServer 輪廓＝真造字，已入表。
其中三筆同 §6.1 的異體正規化：字形 `叁`→`參`、`数`→`數`、`运`→`運`，全屬一般詞彙非人名。

3 個 F0xx（`F02A` `F099` `F0B7`）的 Wingdings 字形名是 `envelopeback`／`budleafse`／`oneoclock`
——**與用法完全對不上**，原文一律當條列項目符號用。曾考慮照 Symbol 解讀（Symbol 0xB7 = `•`
BULLET，`F0B1` 有取 Symbol 的先例），使用者判讀後決定三個一律視為空白。請勿「修正」成項目符號。

驗收：`origin/data` 全 11 學期實跑 `scan_canonical` → **0 命中**。`PUA_MAP` 現共 **107 碼位**
（學校造字 73／Word 符號 34）。

### 6.5 走過的冤枉路（給未來 session）
