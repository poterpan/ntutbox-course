...
cron (GH Actions, 04:00 Asia/Taipei)
  ...
  → git commit（分兩判斷：catalog/classes 結構有變才 commit；enrollment snapshot 視需要 commit；[skip ci]、bot 身分）
...
- **既有 11 學期遷移**（離線、不重爬）：擴充 `rederive`/新增 `build-canonical`：把現有資料的 enrollment 從 catalog 抽出 → 種初始 `enrollment/{today}.ndjson`；catalog 去 volatile + 去時間戳；classes.json 落到 canonical。
...
- 觸發：`schedule: cron '0 20 * * *'`（= 04:00 Asia/Taipei，台北無 DST）＋ `workflow_dispatch`（輸入 `terms`、`force`）。
...
- 步驟：checkout（完整 canonical 歷史）→ setup Python 3.12 + 裝 `crawler` → **驗證 `terms` 輸入**（regex `^\d{3}-[123](:\d{3}-[123])?(,...)?$`，擋注入）→ 讀 `vars.ACTIVE_TERMS`（預設 `115-1`）→ `ntut_catalog crawl --terms $TERMS`（含寫 enrollment snapshot）→ **資料紅線掃描**（擋學號樣式/cookie/session/token/完整 HTML 錯誤頁進 canonical，特別查 `raw_fields`）→ commit：
  - `catalog.ndjson`/`classes.json` 有結構 diff → commit（訊息 `data(catalog): ...`）。
  - enrollment snapshot → commit（訊息 `data(enrollment): {term} {date} [skip ci]`）。
  - 兩者皆無 → 不 commit。
  → `git pull --rebase` 後 `git push`（Codex：防 reject）→ `infra/publish.py --terms $TERMS`（含 build-v1 全量 + quality gate + 原子發佈）。
- **只爬 active term**：歷史學期凍結，不每日重爬 → 對學校 ~5 分鐘輕負載。歷史全量重爬走 `workflow_dispatch` + `force`。
- Secrets：`CLOUDFLARE_API_TOKEN`、`CLOUDFLARE_ACCOUNT_ID`（GH secrets）；`ACTIVE_TERMS`、bucket 名、quality-gate 門檻走 repo variables。

**驗收**：`workflow_dispatch` 手動跑 active term 成功；無結構變更時只產 enrollment commit、不產 catalog commit；同日 rerun 不 push 衝突；R2 出現新物件且 manifest 完整。
...
1. `gh repo create`（**public**，依 CLAUDE.md）+ 設 remote + push（含 P0 已 commit 歷史）。
...
## 建置順序
A（enrollment 分離 + canonical classes + 遷移 + build-v1）→ B（publish.py + quality gate + 原子發佈，可 --dry-run 先驗）→ 資源建立（確認後）→ C（workflow + 紅線掃描 + concurrency）→ 端到端：dispatch 跑一次 active term，驗 R2 + git。
