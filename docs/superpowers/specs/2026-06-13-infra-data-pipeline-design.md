...
- **自動偵測當前學期**（比照 gnehs）：新增 `detect_current_term()`（讀 `QueryCurrPage.jsp` 的 `select[name=year]`/`select[name=sem]` selected option）+ CLI `current-term` 子命令。workflow 預設用它；`ACTIVE_TERMS` 可覆寫。
  - **學期生命週期前提**（使用者確認）：學校在**學期末**才上架下學期課表 → 自動偵測到的「最新學期」正是唯一還在變動的（結構收尾 + 預選人數跳動）；開學後課綱凍結、不增減課 → 舊學期 catalog 不再變，daily 結構 diff 幾乎恆空、commit 主要來自 enrollment snapshot。故「只爬自動偵測的當前學期」足夠且正確。
...
- 步驟：
  ...
  3. 決定要爬的學期：若 `vars.ACTIVE_TERMS` 有值（或 dispatch `terms` 輸入）→ 用它（先過 regex `^\d{3}-[123](:\d{3}-[123])?(,...)?$` 擋注入）；否則 **`ntut_catalog current-term` 自動偵測**（讀 QueryCurrPage 下拉 selected year/sem，比照 gnehs `fetchYearSem`）。
...
## 選課季 enrollment 快速刷新（Phase D — **本次上線範圍**）
> 修訂 2026-06-13：原列 fast-follow，但**現正值 115-1 預選（6/8–6/19）**，每小時不上線即流失本次預選人數時序（事後無法重建）。系統開發已晚，決定一次上線、再轉做 web。
選課季 enrollment 快速跳動，但**規劃用途 15–60 分鐘新鮮度即足**（送件權威在 App 打 cwish live；catalog 無容量上限，只能顯示絕對人數）。學校此時負載最高，**不可高頻狂掃**（gnehs：太快會被封）。
- **① 爬蟲 enrollment-only 路徑**：`refresh-enrollment` 子命令 / `crawl_enrollment()`——Subj-2 取系所 + 各系所 QueryCourse 讀人/撤（~62 請求），**跳過** Subj-3 班級清單、13 學制查詢、catalog/classes 重產。只產 `EnrollmentLatest` → 寫 snapshot → build_v1（catalog 不變、enrollment.json 更新）。
- **② `crawl-enrollment.yml`**：hourly cron（`0 * * * *`）+ 窗口閘 repo variable `ENROLLMENT_FAST_UNTIL=<YYYY-MM-DD>`：`今天 > 該日 → 立即 no-op`（過期自動停）。自動偵測當前學期。與 `crawl.yml` 共用 `concurrency: data-pipeline`（序列化、不撞 data branch/R2）。
- **③ snapshot 顆粒**：snapshot 檔名帶**小時**（`enrollment/{YYYY-MM-DDTHH}.ndjson`）→ 同小時重跑覆寫（≤24 檔/日）、跨小時新檔＝時序；build_v1 取最新（排序）還原 enrollment.json。daily 仍寫 date-only 一筆。
- **④ 快取**：enrollment.json `max-age=300`（5 分）已夠（hourly 刷新）；如需更即時可再降，暫不加旋鈕。
...
- ✅ **自動偵測當前學期 + 學期生命週期**（使用者，比照 gnehs）：`detect_current_term()` 讀 QueryCurrPage selected year/sem，`ACTIVE_TERMS` 可覆寫。學期末才上架下學期、開學後凍結 → 只爬自動偵測學期足夠。已納入 Phase A/C。

## gnehs 對照（實機讀 repo 2026-06-13）
驗證一致：daily 只爬當前學期（`fetchYearSem` 自動偵測）、全 20 年是手動 `fetchAll.js`、資料放 gh-pages branch + `keep_files`（＝我們 orphan data branch + R2 歷史物件保留）。我們改進兩點：① 不按學制切檔（避免跨檔重複列，§4.7）；② enrollment 抽離 catalog（gnehs inline people → 每日整檔 churn；我們 catalog 純結構、只 snapshot churn）。採納一點：自動偵測當前學期。
